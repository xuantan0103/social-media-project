...
function FriendRequests({friendsRequest}) {
  ...
  const Button = (props) => {
      ...
      if(!isAccept){
        return(
          <React.Fragment>
              <button style={{backgroundColor: 'red', color: 'white'}}>Cancelled</button>
          </React.Fragment>
        )
      }
      ...
      return(
        <React.Fragment>
          ...
          <button onClick={() => onSubmitFriendRequest('remove', props.user.id)}>Cancle</button>
        </>
          :
            <button style={{backgroundColor: 'rgba(0,128,128)', color: 'white'}}>Friend</button>
          }
        </React.Fragment>
      )
  }
  ...
  const FriendRequests = () => {
    return (<> < div className = "friends-requests-all" style = {{ marginTop: "65px" }} > <div className="friend-requests-container">
      <div className="friend-requests-title">
        <h4>
        Reply {list.length} Your friend request</h4>
        <Link to="/friend-sent">
          <span>
          View Sent Requests</span>
        </Link>
